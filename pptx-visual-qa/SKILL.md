...
- Scripted `open` on a **freshly launched Keynote** can make Keynote quit mid-command (`Connection is invalid (-609)`, with a *Keynote quit unexpectedly* dialog) or return an import placeholder (`unmerge id … doesn't understand the "close" message (-1708)`), for files that open fine otherwise (2026-09-22). `render_pptx.sh` falls back to `open -a Keynote <file>`, an eight-second wait, then `export front document`; write that route by hand if scripting Keynote outside the script.
